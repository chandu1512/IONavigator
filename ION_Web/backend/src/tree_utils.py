...
class DiagnosisTable:
    def __init__(self):
        self.nodesTable = pd.DataFrame(
            columns=['id', 'name', 'content', 'child', 'step', 'module', 'object'])
        self.nodes = {}
    ...
    def add_node(self, node):
        ...
        self.nodesTable = pd.concat(
            [self.nodesTable, new_entry], ignore_index=True)
        self.nodes[node.id] = node

    def update_node_child(self, node_id, child_id):
        self.nodesTable.loc[self.nodesTable['id']
    ...
    def to_tree_json(self):
        ...
        final_diagnosis_node = self.nodesTable[self.nodesTable['step']
                                               == 'final_diagnosis']
...
def parse_dir_tree(directory_path):
    ...
    step_order = ['summary_fragments', 'rag_diagnoses',
                  'intra_module_merges', 'inter_module_merges', 'final_diagnosis']
...
def parse_summary_fragments(directory_path, diagnosis_table):
    ...
    for summary_fragment in os.listdir(summary_fragments_path):
        with open(os.path.join(summary_fragments_path, summary_fragment), 'r') as f:
            ...
            DiagnosisTreeNode(summary_fragment, summary_fragment_content,
                              'summary_fragments', module_name, diagnosis_table)
    ...
    for rag_diagnosis in os.listdir(rag_diagnoses_path):
        if rag_diagnosis.endswith('.json'):
            with open(os.path.join(rag_diagnoses_path, rag_diagnosis), 'r') as f:
                ...
                summary_node = diagnosis_table.nodesTable[diagnosis_table.nodesTable['name']
                                                          == corresponding_summary]
                summary_node = summary_node[summary_node['step']
                                            == 'summary_fragments'].iloc[0]
                rag_diagnosis_node = DiagnosisTreeNode(
                    rag_diagnosis, rag_diagnosis_content, 'rag_diagnoses', summary_node['module'], diagnosis_table)
...
def parse_intra_module_merge(directory_path, diagnosis_table):
    ...
    intra_module_merge_path = os.path.join(
        directory_path, "intra_module_merges")
    for dir_name in os.listdir(intra_module_merge_path):
        if not "final" in dir_name:
            module_name = dir_name
            # convert subdir to int and sort
            subdirs = [int(subdir) for subdir in os.listdir(
                os.path.join(intra_module_merge_path, dir_name))]
            subdirs.sort()
            ...
            for file in os.listdir(os.path.join(intra_module_merge_path, dir_name, str(subdirs[0]))):
                if file.endswith('.json'):
                    ...
                    if idx != max_idx:
                        ...
                        rag_diagnosis1 = module_nodes.loc[module_nodes['idx']
                                                          == idx, 'id'].iloc[0]
                        rag_diagnosis1_node = diagnosis_table.nodes[rag_diagnosis1]
                        rag_diagnosis2 = module_nodes.loc[module_nodes['idx']
                                                          == idx + 1, 'id'].iloc[0]
                        rag_diagnosis2_node = diagnosis_table.nodes[rag_diagnosis2]
                        intra_module_merge_node = DiagnosisTreeNode(
                            file, intra_module_merge_content, 'intra_module_merges', module_name, diagnosis_table)
                        rag_diagnosis1_node.add_child(
                            intra_module_merge_node, diagnosis_table)
                        rag_diagnosis2_node.add_child(
                            intra_module_merge_node, diagnosis_table)
                        if module_name == "POSIX":
                            tree_logger.info(
                                f"adding {intra_module_merge_node.name} as child to {rag_diagnosis1_node.name}")
                            tree_logger.info(
                                f"adding {intra_module_merge_node.name} as child to {rag_diagnosis2_node.name}")
                    else:
                        rag_diagnosis1 = module_nodes.loc[module_nodes['idx']
                                                          == idx, 'id'].iloc[0]
                        rag_diagnosis1_node = diagnosis_table.nodes[rag_diagnosis1]
            ...
            for subdir in subdirs[1:]:
                ...
                previous_merge_nodes = diagnosis_table.nodesTable[
                    diagnosis_table.nodesTable['module'] == module_name]
                previous_merge_nodes = previous_merge_nodes[previous_merge_nodes['step']
                                                            == 'intra_module_merges']
                # find where child is None
                previous_merge_nodes = previous_merge_nodes[previous_merge_nodes['child'].isna(
                )]
                previous_merge_nodes['idx'] = previous_merge_nodes['name'].apply(
                    lambda x: int(x.split('.')[0]))
                # sort by idx
                previous_merge_nodes = previous_merge_nodes.sort_values(
                    by='idx')
                for file in os.listdir(os.path.join(intra_module_merge_path, dir_name, str(subdir))):
                    if file.endswith('.json'):
                        ...
                        if len(previous_merge_nodes) > 1:
                            ...
                            intra_module_merge_node = DiagnosisTreeNode(
                                file, intra_module_merge_content, 'intra_module_merges', module_name, diagnosis_table)
                            previous_merge_node1_obj.add_child(
                                intra_module_merge_node, diagnosis_table)
                            previous_merge_node2_obj.add_child(
                                intra_module_merge_node, diagnosis_table)
                            if module_name == "POSIX":
                                tree_logger.info(
                                    f"adding {intra_module_merge_node.name} as child to {previous_merge_node1_obj.name}")
                                tree_logger.info(
                                    f"adding {intra_module_merge_node.name} as child to {previous_merge_node2_obj.name}")
                        else:
                            ...
                            intra_module_merge_node = DiagnosisTreeNode(
                                file, intra_module_merge_content, 'intra_module_merges', module_name, diagnosis_table)
                            previous_merge_node_obj.add_child(
                                intra_module_merge_node, diagnosis_table)
                            if module_name == "POSIX":
                                tree_logger.info(
                                    f"adding {intra_module_merge_node.name} as child to {previous_merge_node_obj.name}")
                        tree_logger.info(
                            f"previous_merge_nodes: {previous_merge_nodes}")
    ...
    for dir_name in os.listdir(intra_module_merge_path):
        if "final" in dir_name:
            ...
            for file in os.listdir(final_dir):
                ...
                intra_module_merge_nodes = diagnosis_table.nodesTable[
                    diagnosis_table.nodesTable['module'] == module_name]
                print("intra_module_merge_nodes", intra_module_merge_nodes)
                intra_module_merge_nodes = intra_module_merge_nodes[
                    intra_module_merge_nodes['step'] == 'intra_module_merges']
                print("intra_module_merge_nodes", intra_module_merge_nodes)
                intra_module_merge_nodes = intra_module_merge_nodes[intra_module_merge_nodes['child'].isna(
                )]
                print("intra_module_merge_nodes", intra_module_merge_nodes)
                final_intra_module_merge_node = DiagnosisTreeNode(
                    file, intra_module_merge_content, 'intra_module_merges', module_name, diagnosis_table)
                print("intra_module_merge_nodes", intra_module_merge_nodes)
                ...
                for _, node in intra_module_merge_nodes.iterrows():
                    ...
                    node_obj.add_child(
                        final_intra_module_merge_node, diagnosis_table)
                    if module_name == "POSIX":
                        tree_logger.info(
                            f"adding {final_intra_module_merge_node.name} as child to {node_obj.name}")


def parse_inter_module_merge(directory_path, diagnosis_table):
    inter_module_merge_path = os.path.join(
        directory_path, "inter_module_merges")
    subdirs = [int(subdir) for subdir in os.listdir(inter_module_merge_path)]
    subdirs.sort()
    module_diagnoses = diagnosis_table.nodesTable[diagnosis_table.nodesTable['step']
                                                  == 'intra_module_merges']
    ...
    for file in os.listdir(os.path.join(inter_module_merge_path, str(subdirs[0]))):
        if file.endswith('.json'):
            ...
            module_name = file.split('.')[0]
            inter_module_merge_node = DiagnosisTreeNode(
                file, inter_module_merge_content, 'inter_module_merges', module_name, diagnosis_table)
            for module in modules_merged:
                module_node = module_diagnoses[module_diagnoses['name']
                                               == module+".json"]
                module_node_obj = diagnosis_table.nodes[module_node['id'].iloc[0]]
                module_node_obj.add_child(
                    inter_module_merge_node, diagnosis_table)
...
def parse_final_diagnosis(directory_path, diagnosis_table):
    ...
    final_diagnosis_node = DiagnosisTreeNode(
        file_name, final_diagnosis_content, 'final_diagnosis', 'final_diagnosis', diagnosis_table)
    # get all nodes with step 'inter_module_merge' that have no child
    inter_module_merge_nodes = diagnosis_table.nodesTable[diagnosis_table.nodesTable['step'].isin(
        ['inter_module_merges', 'intra_module_merges'])]
